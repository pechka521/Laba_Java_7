package com.example.lab7.aspect;
...
public class LoggingAspect {
    ...
    @Before("execution(* com.example.lab7.controller.*.*(..))")
    public void logBeforeController(JoinPoint joinPoint) {
        logger.info("Entering controller method: {} with arguments: {}",
                joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()));
    }

    @AfterReturning(pointcut = "execution(* com.example.lab7.controller.*.*(..))", returning = "result")
    public void logAfterController(JoinPoint joinPoint, Object result) {
        logger.info("Exiting controller method: {} with result: {}",
                joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.example.lab7.controller.*.*(..))", throwing = "exception")
    ...
    @Before("execution(* com.example.lab7.service.*.*(..))")
    public void logBeforeService(JoinPoint joinPoint) {
        logger.debug("Entering service method: {} with arguments: {}",
                joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()));
    }

    @AfterReturning(pointcut = "execution(* com.example.lab7.service.*.*(..))", returning = "result")
    public void logAfterService(JoinPoint joinPoint, Object result) {
        logger.debug("Exiting service method: {} with result: {}",
                joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.example.lab7.service.*.*(..))", throwing = "exception")
